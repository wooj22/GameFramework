...
// A. 위 코드처럼 해도 됨. 가능한 방식임 (다중 인스턴스일때는 활용 불가능)
// Q. 그렇다면 왜 포인터를 저장하는 방식이 쓰일까?
// A. 윈도우 프로시저는 글로벌 콜백 함수로 멀티 윈도우를 만들거나 
//    여러개의 윈도우를 제어할때는 윈도우 클래스가 하나가 아닌 다중 인스턴스일 것이고
//	  그때는 윈도우 각각의 this를 저장해야 하기 때문에 확장성의 방면에서 쓰임
